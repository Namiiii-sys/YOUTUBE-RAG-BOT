...
const Chatinterface = () => {
  ...
  const handleAsk = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/query?video_url=${encodeURIComponent(window.location.href)}&question=${encodeURIComponent(question)}`,
        {
          method: "POST",
        }
      );
  ...
  return (
    <div className="w-[450px] h-[500px] px-6 py-4 overflow-hidden bg-gray-100 rounded-lg shadow-md flex flex-col">
      ...
      <div className="h-3/4 w-full border border-indigo-950 rounded-lg p-3 overflow-y-auto space-y-2">
        {chat.map((msg, i) => (
          <div
            key={i}
            className={`p-2 rounded-2xl text-sm w-fit max-w-[75%] ${msg.type === "user" ? "bg-black ml-auto text-white" : "bg-gray-200 text-black"}`}
          >
            {msg.text}
          </div>
        ))}
